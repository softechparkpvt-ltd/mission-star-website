		...
		if( isset( $_POST['wp-travel-engine-confirmation-submit'] ) && isset( $_SESSION['trip-id'] ) )
		{
			...
			<table>
				<thead>
					...
					{	?>
						<th><?php _e( 'Extra Service(s):','wp-travel-engine' ); ?></th>
					<?php
					}	
					?>
				</thead>
				<tbody>
					...
					if( isset( $_SESSION['extra_service'] ) && $_SESSION['extra_service']!='' )
					{
						...
						foreach ( $_SESSION['extra_service'] as $key => $value ) {
							if( isset( $wp_travel_engine_trip_setting['extra_service'][$value] ) && $wp_travel_engine_trip_setting['extra_service'][$value]!='' )
							{
								echo '<span class="extra-service-name">'.$wp_travel_engine_trip_setting['extra_service'][$value].': </span>';

								echo '<span class="extra-service-cost">'.$_SESSION['extra_service'][$key].' X '.esc_attr($currency).esc_attr( $obj->wp_travel_engine_price_format( $_SESSION['extra_service_name'][$key] ) ).esc_attr($code).'</span>';
							}
						}
						echo '</td>';		
					}
				...
				<table>
					...
					<tbody>
						...
						if( isset( $_SESSION['extra_service'] ) && $_SESSION['extra_service']!='' )
						{
							...
							foreach ( $_SESSION['extra_service'] as $key => $value )
							{
								if( isset( $wp_travel_engine_trip_setting['extra_service'][$value] ) && $wp_travel_engine_trip_setting['extra_service'][$value]!='' && isset( $_SESSION['extra_service'][$key] ) && $_SESSION['extra_service'][$key]!='0' ){

										echo '<span class="extra-service-name">'.isset( $wp_travel_engine_trip_setting['extra_service'][$value] ) ? esc_html($wp_travel_engine_trip_setting['extra_service'][$value]):'N/A'.': </span>';
										echo '<span class="extra-service-cost">'.$_SESSION['extra_service'][$key].' X '.esc_attr($currency).esc_attr( $obj->wp_travel_engine_price_format( $_SESSION['extra_service_name'][$key] ) ).esc_attr($code).'</span>';
